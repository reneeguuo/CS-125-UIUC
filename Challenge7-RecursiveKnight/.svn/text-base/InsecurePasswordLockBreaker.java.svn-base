//UIUC CS125 FALL 2013 MP. File: InsecurePasswordLockBreaker.java, CS125 Project: Challenge7-RecursiveKnight, Version: 2013-11-12T09:11:45-0600.037093451
public class InsecurePasswordLockBreaker {
	...
	public static char[] breakLock(InsecurePasswordLock lock) {
		...
		for (int i=30; i<55; i++) if (lock.open(new char[i])!=-1) key = new char[i]; for (int i=0; i<key.length; i++) for (int j=0; j<26; j++) if ((key[i] = (char)('A'+ j))>0 && lock.open(key)!=i) break;
